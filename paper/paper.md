...
*Figure 1: The input 3D image is binarized, then smoothed with a Gaussian smoothing kernel to create a 3D floating point array. A mesh is then generated and improved using python VTK [@vtkBook], before being rasterized to the target resolution to create the upsampled image.*

This section introduces an algorithm that utilizes a mesh-based method for the upsampling process. The workflow of the algorithm is depicted in Figure 1. An image with a single label can be equated to a binary image. To begin, the algorithm converts the binary image to a floaing point array, then smooths the image using a Gaussian smoothing kernel [@virtanen2020scipy, @keerthi2003asymptotic]. Smoothing is implemented on the basis that smooth surfaces are a better approximation of the natural shape of a biological object. Next, an isosurface is extracted from the smoothed image, generated from points within the volume with a constant value. Subsequently, the algorithm generates a triangulated free-space surface mesh isosurface through vtkFlyingEdges3D [@vtkBook, @schroeder2015flying]. A hole-filling function is implemented to improve the mesh quality. Finally, the free-space mesh is rasterized to the output grid with the required discretization by doing a pixelwised operation of vtk distance filter [@vtkBook, @quammen2011boolean] for accurate ultrasound simulation.
...
In this algorithm, an evaluation method for measuring object complexity and matching the corresponding upsampling parameters is proposed. An ideal sphere is defined as the simplest shape, as it has the least surface area for a given volume. While the surface area of a shape is proportional to the square of its radius, while the volume is proportional to the cube of its radius. Therefore, a size-independent shape complexity parameter, AV, can be defined as follows.

$$%AV = \frac{\sqrt{A}}{\sqrt[3]{V}}$$

This AV value is then matched to sigma based on empirical observation in Figure 3. The general principle behind this relationship is grounded in the understanding that as a shape becomes more complex, greater smoothing is required for effective upsampling.

![Matching AV to sigma](figure/AVtoSigma.svg)
*Figure 3: Matching AV to sigma. Most medical images used as examples in this project have an AV around 4. The majority of sigma values are matched to AVs in the range of 2.9 to 4.5.*

In addition to this matching, AV is also used to generate a non-isotropic Gaussian by applying the ratio of AV in three different directions. This approach can also be used to optimize the isovalue by identifying the AV of the shape enclosed by the isosurface that most closely matches the original shape.

This upsampling process does not provide a globally optimal parameter setting but offers a reasonable suggestion with acceptable performance. It is designed for situations where high-resolution ground truth is unavailable or when grid search is too time-consuming. Users can apply the suggested parameter setting to perform their upsampling.

# Application

We used the code-defined test object displayed in Fig. 3 to identify optimal $\sigma$ and $I$ values. The test object is generated by subtracting smaller spheres from a larger sphere. Spheres are generated using the $makeSphere$ function from the k-Wave[@treeby2010k] acoustic simulation toolbox. The base sphere has a radius of $r$ and is centered at $(0,0,0)$. Six smaller spheres with radii of $0.75r, 0.67r, 0.5r, 0.33r, 0.25r, 0.2r$ are subtracted at $(\pm r,0,0), (0,\pm r, 0), (0,0,\pm r)$ respectively. The radius $r$ was set to 20 voxels. The spherical subtractions from the base sphere generate sharp edges, similar to certain bony anatomical features that need to be preserved during upsampling for simulation accuracy. Some upsampling algorithms may have an advantage in upsampling simple geometric shapes. To counteract this, the test object has convex components, concave components, and edges of varying ‘sharpness’, with the aim of representing a broad range of possible biological features. 
...
*Figure 4: 'Complex' Test Object: three-dimensional isometric projection, b) top view, c) front view, and d) side view of the complex test object.*
...
*Figure 5: Percentage upsampling error for the mesh-based upsampling algorithm versus two naive upsampling algorithms: trilinear and nearest-neighbour upsampling. The errors are evaluated across a range of upsampling scales.*
